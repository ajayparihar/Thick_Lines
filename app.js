...
let currentTool = 'pen';
let eraserSize = 50; // Default eraser size
  ...
  try {
    ...
    document.addEventListener('mouseup', function(e) {
      ...
      if (e.button === 1 && isMiddleMouseDown) {
        ...
        const moveThreshold = 5; // Allow small movement (5px) to still count as a click
...
function setTool(tool) {
  ...
  if (tool === 'pen') {
    ...
    // Update the pen button to show the current size
    document.getElementById('penBtn').innerHTML = `<i class="fas fa-pencil-alt"></i> Pen <span class="size-indicator">${penSize}px</span>`;
  } else if (tool === 'eraser') {
    document.getElementById('eraserBtn').classList.add('active');
    // Update the eraser button to show the current size
    document.getElementById('eraserBtn').innerHTML = `<i class="fas fa-eraser"></i> Eraser <span class="size-indicator">${eraserSize}px</span>`;
  }
}
...
function drawEraserPath(prevPoint, currentPoint) {
  ...
  // Use destination-out for eraser effect
  ctx.globalCompositeOperation = 'destination-out';
  ctx.strokeStyle = 'rgba(0, 0, 0, 1)'; // Any color with destination-out
  ctx.lineWidth = eraserSize;
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  
  // Draw the eraser line
  ctx.beginPath();
  ctx.moveTo(prevPoint.x, prevPoint.y);
  ctx.lineTo(currentPoint.x, currentPoint.y);
  ctx.stroke();
